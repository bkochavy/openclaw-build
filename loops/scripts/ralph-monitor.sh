...
while IFS= read -r SESSION; do
  ...
  if echo "$OUTPUT" | grep -q 'EXITED:'; then
    ...
    if [ "$ALREADY_NOTIFIED" != "true" ]; then
      ...
      if [ -n "$OPENCLAW_BIN" ]; then
        "$OPENCLAW_BIN" system event --text "RALPH POST-COMPLETION: Session '${SESSION}' finished (exit ${EXIT_CODE}) in ${REPO}. Run the Post-Completion Protocol from AGENTS.md now." --mode now >/dev/null 2>&1 || true
      fi
