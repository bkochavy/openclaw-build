...
**Task sizing:** Target 3-5 min/task. >7 min means it is too big - split it.
...
Then edit `.ralphy/config.yaml` (project name, language, framework, test/lint/build commands, boundaries for files agents should never touch). Template at `templates/`.

Create `CLAUDE.md` (for Claude Code) or `AGENTS.md` (for Codex) from templates in `templates/`.
...
**CRITICAL: `--prd` and `--yaml` go BEFORE `--`.** Everything after `--` is passed to the engine. Putting ralphy flags after `--` is a silent failure - the engine ignores them and ralphy runs without a PRD.
...
9. **Never tell the operator "launched" without confirming step 8.**
...
### When to parallelize
- Yes: UI components, multi-service, test suites, model/migration creation
- No: sequential dependencies (auth -> routes -> admin), single-file fixes

## PRD Format

### Critical: Checkbox Parsing Rules
...
`--max-iterations` = 1.5-2x your task count. Template at `templates/PRD.md.template`.
...
- **Wrong task count:** Count `- [ ]` lines in your PRD - that is your task count. If it does not match what ralphy reports, you have nested checkboxes or stray `- [ ]` in non-task sections.
